...
class WordCompletionCubit extends Cubit<GameState> {
  ...
  void selectOption(int selectedIndex) {
    ...
    final isCorrect =
        currentQuestion.correctChoice == currentQuestion.choices[selectedIndex];
    final int correctAnswerIndex =
        currentQuestion.choices.indexOf(currentQuestion.correctChoice);

    if (isCorrect) {
      score++;
      emit(CorrectAnswerState(
          questionIndex: index, selectedIndex: selectedIndex));
    } else {
